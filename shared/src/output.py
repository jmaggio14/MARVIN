...
def prevent_overwrite(filename,create_file=False):
    ...
    NOTE:
        this function is referenced (as of Nov, 2017) in marvin's init()
        function and therefore cannot be modifed to require
        post-initialization global variables

        This function creates unique filenames by creating them and
        checking their current existence. THIS CAN BE A VERY SLOW
    ...
    while file_exists:
        out_filename = "{base}({num}){ext}".format(base=base_filename,
                                        num=marvin.make_numbered_prefix(num,0),
                                        ext=extension)
        num += 1
        file_exists = os.path.exists(out_filename)
