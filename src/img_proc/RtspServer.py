...
class RtspServer(object):
    def __init__(self,
        ...
        self.fps = fps
        self.__gstreamer_kwargs = {"host":host,"port":port,"threads":threads}
        self.gstreamer_string = """appsrc ! videoconvert ! x264enc ! video/x-h264 threads={threads}! rtph264pay ! udpsink host={host} port={port}""".format(**self.__gstreamer_kwargs)
#
#
    def __initializeWriter(self,width,height):
        ...
        self.streamer = cv2.VideoWriter( filename=self.gstreamer_string,fourcc=0,fps =self.fps,frameSize=(width,height) )
        marvin.Status.info("initialized RtspStreamer operating on {threads} threads via host: {host}:{port}".format(**self.__gstreamer_kwargs))
#
...
#     def __init__(self):
#         gstreamer_string = "gst-launch-1.0 udpsrc port=5000  !  application/x-rtp ! rtph264depay ! avdec_h264 ! xvimagesink"
